...
describe('Bytes', () => {
  it('toBytes: converts null to empty array', () => {
    expect(toBytes(null)).to.be.eql(Buffer.from([]));
  });

  const testCase = 'test_test-testTest';

  it('converts snake to pascal case', () =>
    expect(snakeToPascal(testCase)).to.be.equal('testTest-testTest'));

  it('converts pascal to snake case', () =>
    expect(pascalToSnake(testCase)).to.be.equal('test_test-test_test'));

  it('converts BigNumber to Buffer', () =>
    expect(toBytes(new BigNumber('1000')).readInt16BE()).to.be.equal(1000));
