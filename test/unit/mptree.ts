...
describe('Merkle Patricia Tree', () => {
  ...
  it('throws exception if payload is invalid', () => {
    ...
    expect(() => new MPTree(brokenValue)).to.throw(MerkleTreeHashMismatchError, 'Node hash is not equal to provided one');
    ...
    expect(() => new MPTree(wrongRootHash)).to.throw(MissingNodeInTreeError, 'Can\'t find a node by root hash');
