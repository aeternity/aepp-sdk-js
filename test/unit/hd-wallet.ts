...
describe('hd wallet', () => {
  ...
  describe('generateSaveHDWalletFromSeed', () => it('generates encrypted extended wallet key', () => {
    const walletKeys = generateSaveHDWalletFromSeed(testMnemonicSeed, testPassword);
    expect(walletKeys).to.eql(testSaveHDWallet);
  }));

  describe('getSaveHDWalletAccounts', () => it('generates array of accounts', () => {
  ...
  describe('getMasterKeyFromSeed', () => it('generates correct master key', () => {
    const masterKey = getMasterKeyFromSeed(testSeed);
    expect(masterKey).to.eql(testMasterKey);
  }));

  describe('deriveChild', () => it('derives correct child key', () => {
    const childKey = deriveChild(testMasterKey, testChildIndex);
    expect(childKey).to.eql(testChildKey);
  }));

  describe('derivePathFromKey', () => it('derives correct child key by path and key', () => {
    const childKey = derivePathFromKey(testChildPath.replace('m/', '') as '0h', testMasterKey);
    expect(childKey).to.eql(testChildKey);
  }));

  describe('derivePathFromSeed', () => it('derives correct child key by path and seed', () => {
  ...
  describe('check on test vectors for ed25519 from SLIP-0010', () => ([{
    ...
    .forEach(({ seed, tests }, idx) => {
      describe(`vector ${idx + 1}`, () => tests.forEach((test) => it(`chain ${test.path}`, () => {
