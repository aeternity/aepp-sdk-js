...
import {
  MemoryAccount, verifyMessage, InvalidChecksumError, verify,
} from '../../src';
...
describe('MemoryAccount', () => {
  ...
  it('Sign raw data', async () => {
    ...
    expect(signature).to.be.eql(Uint8Array.from([
      113, 154, 121, 195, 164, 141, 153, 234, 196, 82, 120, 31, 198, 179, 208, 138, 88, 63, 98, 221,
      126, 199, 240, 117, 105, 164, 118, 214, 30, 211, 116, 118, 211, 89, 218, 196, 223, 182, 6,
      218, 199, 123, 123, 227, 78, 140, 113, 205, 53, 38, 23, 15, 56, 186, 245, 88, 217, 187, 249,
      219, 99, 74, 255, 5,
    ]));
    expect(verify(data, signature, account.address)).to.be.equal(true);
  });
  ...
  it('Sign message message with non-ASCII chars', async () => {
    ...
    const signature = await account.signMessage(message);
    expect(signature).to.be.eql(Uint8Array.from([
      164, 138, 41, 174, 108, 125, 147, 42, 137, 79, 226, 64, 156, 196, 232, 24, 216, 105, 105, 132,
      212, 15, 198, 226, 48, 119, 36, 2, 115, 211, 90, 24, 242, 213, 206, 221, 215, 32, 192, 137,
      161, 10, 2, 16, 184, 7, 0, 177, 157, 15, 143, 138, 207, 71, 51, 245, 56, 155, 18, 68, 183,
      197, 251, 3,
    ]));
    expect(verifyMessage(message, signature, account.address)).to.be.equal(true);
  });
