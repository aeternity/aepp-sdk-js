...
describe('Contract', function () {
  ...
  describe('precompiled bytecode', () => {
    it('can be invoked', async () => {
      return contract.contractCallStatic(identityContractByteCode, 'sophia', 'main', {args: '42', conformFn: parseInt}).should.eventually.become(42)
    })
  ...
  it.skip('initializes contract state', async () => {
    ...
    return contract.contractCompile(stateContract).then(bytecode => bytecode.deploy({initState: data})).then(deployed => deployed.call('retrieve', '()')).catch(e => { console.log(e); throw e }).should.eventually.become('Hello World!')
  })
})
