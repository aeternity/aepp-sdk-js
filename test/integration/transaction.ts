...
describe('Transaction', function () {
  ...
  const transactions: Array<[string, string, () => Promise<string>]> = [[
    ...
    async () => await aeSdk.buildTx(TX_TYPE.spend, {
    ...
    async () => await aeSdk.buildTx(TX_TYPE.namePreClaim, {
    ...
    async () => await aeSdk.buildTx(
      TX_TYPE.nameClaim, { accountId: senderId, nonce, name, nameSalt, nameFee }
    )
  ], [
    'name update',
    ...
    async () => await aeSdk.buildTx(
      TX_TYPE.nameUpdate, { accountId: senderId, nonce, nameId, nameTtl, pointers, clientTtl }
    )
  ], [
    'name revoke',
    'tx_+E8jAaEB4TK48d23oE5jt/qWR5pUu8UlpTGn8bwM5JISGQMGf7ABoQL1zlEz+3+D5h4MF9POub3zp5zJ2fj6VUWGMNOhCyMYPIYPHaUyOAAA94BVgw==',
    async () => await aeSdk.buildTx(TX_TYPE.nameRevoke, { accountId: senderId, nonce, nameId })
  ], [
    'name transfer',
    ...
    async () => await aeSdk.buildTx(
      TX_TYPE.nameTransfer, { accountId: senderId, nonce, nameId, recipientId }
    )
  ], [
    'contract create',
    ...
    async () => await aeSdk.buildTx(TX_TYPE.contractCreate, {
      nonce,
      ownerId: address,
    ...
    async () => await aeSdk.buildTx(TX_TYPE.contractCall, {
      nonce,
      callerId: address,
      contractId,
      amount,
    ...
    async () => await aeSdk.buildTx(TX_TYPE.oracleRegister, {
    ...
    async () => await aeSdk.buildTx(TX_TYPE.oracleExtend, {
    ...
    async () => await aeSdk.buildTx(TX_TYPE.oracleQuery, {
    ...
    async () => await aeSdk.buildTx(TX_TYPE.oracleResponse, {
      nonce,
      oracleId,
      callerId: address,
      ...responseTtl,
