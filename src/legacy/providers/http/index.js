...
class AeHttpProvider {
  ...
  constructor (host, port, {version = CURRENT_API_VERSION, secured = false, internal = false} = {}) {
    ...
    this.ready = new Promise((resolve, reject) => {
      ...
      const options = internal ? { internalUrl: `${url}/internal/` } : {}

      Ae.create(`${this.protocol}://${this.host}:${this.port}/`, options).then(ae => {
        this.ae = ae
        resolve(true)
      }).catch(e => reject(e))
    })
  ...
  async post (endpoint, data, options) {
    ...
    return axios.post(fullUrl, data, {headers: headers})
  }
}
