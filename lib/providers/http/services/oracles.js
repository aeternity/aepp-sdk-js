...
class Oracles extends HttpService {
   ...
   * @param options
   */
  async register(queryFormat, responseFormat, queryFee, ttl, fee, privateKey, options) {
    let payload = {
    ...
    let {data} = await this.client.post(`tx/oracle/register`, payload)
   ...
   * @param options
   */
  async query(oracleId, queryFee, queryTtl, responseTtl, fee, query, privateKey, options = {}) {
    let payload = {
      'response_ttl': {
        'type': 'delta',
        'value': responseTtl
      },
      'sender': options.sender || await this.client.accounts.getPublicKey(),
      'query': query,
      'query_ttl': {
        'type': 'delta',
        'value': queryTtl
   ...
   * @param privateKey
   */
  async respond(queryId, fee, response, privateKey) {
    let payload = {
      "oracle": null,
      'query_id': {},
      'response': 'response',
    ...
    let {data} = await this.client.post(`tx/oracle/respond`, payload)
