...
class Base extends HttpService {
  ...
  async spend(recipient, amount, fee, options) {
    // try {
      ...
      if (options && options.privateKey) {
        data = await this.client.base.getSpendTx(recipient, amount, options)
        await this.client.tx.sendSigned(data.tx, options.privateKey, options)
        return data
      } else {
        throw new Error('Private key is not set')
      }
    // } catch (e) {
    //   throw new Error(`Could not transfer ${amount}`)
    // }
  }
  ...
  async getSpendTx(recipient, amount, options = {}) {
    let payload = {
      'amount': amount,
      'sender': options && options.sender,  // || await this.client.accounts.getPublicKey (),
