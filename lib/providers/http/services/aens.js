...
class AENS extends HttpService {
  ...
  async preClaim(commitment, fee, options = {}) {
    if (options && options.privateKey) {
      ...
      let {tx} = await this.client.post('tx/name/preclaim', payload)
      return await this.client.tx.sendSigned(tx, privateKey)
    } else {
  ...
  async claim(name, salt, fee, options = {}) {
    if (options && options.privateKey) {
      ...
      let {tx} = await this.client.post('tx/name/claim', payload)
      return await this.client.tx.sendSigned(tx, options.privateKey)
    } else {
      let {data} = await this.client.post (
        'name-claim-tx',
  ...
  async transfer(nameHash, recipient, fee = 1, options = {}) {
    ...
    if (options && options.privateKey) {
      ...
      let {tx} = await this.client.post('tx/name/transfer', payload)
      return await this.client.tx.sendSigned(tx, options.privateKey)
    }
    let {data} = await this.client.post (
      'name-transfer-tx', payload, {internal: true})
    return data['name_hash']
  }
  ...
  async revoke(nameHash, fee = 1, options = {}) {
    ...
    if (options && options.privateKey) {
      ...
      let {tx} = await this.client.post('tx/name/revoke', payload)
      return await this.client.tx.sendSigned(tx, privateKey)
    } else {
      let {data} = await this.client.post (
        'name-revoke-tx',
   ...
   * @returns {Promise<*>}
   */
  async fullClaim(domain, preClaimFee, claimFee, salt) {
    // get a random salt
