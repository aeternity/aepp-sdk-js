...
class AENS {
  ...
  async fullClaim(domain, client, preClaimFee, claimFee) {
    // get a random salt
    ...
    // preclaim the domain
    await client.aens.preClaim(commitment, preClaimFee)
    // wait one block
    await client.base.waitNBlocks(1)
    // claim the domain
    let nameHash = await client.aens.claim(domain, salt, claimFee)
    return nameHash
  }
