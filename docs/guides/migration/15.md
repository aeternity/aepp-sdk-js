...
- The `fee`/`gasPrice` default is raised to the minimum gas price the miner of the connected node
  accepts, and a value provided in options is checked against it, so a transaction is priced to be
  mined by the node it is submitted to. On a node whose miner minimum is above the consensus
  minimum this costs more than before.
...
+ const gasPrice = getFloorGasPrice(await getCachedProtocolParameters(node));
```

`MIN_GAS_PRICE` is the price a node accepts by default, so `getFloorGasPrice` is the replacement
and not the consensus minimum on its own — that one is a thousand times lower on mainnet, and node
refuses a transaction priced at it with `too_low_gas_price_for_miner`.
